...
function ImagePicker({ initialImages }: { initialImages: string[] }) {
  ...
  useEffect(() => {
    async function fetchImages() {
      const initialImageItems = await Promise.all(
        initialImages.map(async (url, i) => {
          ...
          return {
            file: new File([blob], `product-image-${i}`, {
              type: type || "image/png",
            }),
            url,
          };
  ...
  return (
    <div className="flex w-full gap-5 overflow-x-auto border border-dashed border-slate-300 p-5">
      {images.map((image, index) => (
        <div
          key={index}
          className="group relative flex h-32 w-32 shrink-0 items-center justify-center bg-slate-200"
        >
          <Image
            src={image.url}
